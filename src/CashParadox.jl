...
function figsub5(irflag::Int64 , flag::Int64, model::Int64)
    ...
    p2=plot(xgrid,vθt,color=:red,label="fitted value",ylims=(0,1),xlabel="year",ylabel="\\theta")
...
function figsubA2(irflag::Int64 , flag::Int64, model::Int64)
    ...
    p2=scatter(vρ1,vθ1,label ="Corr.Coef. before 1990=$cor1",ylim=(0,1),xlabel="Simulated \\rho", ylabel="Simulated \\theta")
...
function Table1(x)
  ...
  θ=zeros(12)
    for mp in 1:3
        ...
        for country in 0:3
            (α[country*3+mp],s[country*3+mp],Xstar[country*3+mp],rmsd[country*3+mp],cor_rho[country*3+mp],cor_theta1[country*3+mp],cor_theta2[country*3+mp],qstar[country*3+mp],θ[country*3+mp])=round.(Calibrate(1,country,model),digits=3)
        end 
    end
    df.α=α
    df.s=s
...
function Table2(x)
  ...
  θ=zeros(12)
    for mp in 1:3
        for country in 0:3
            (α[country*3+mp],s[country*3+mp],Xstar[country*3+mp],rmsd[country*3+mp],cor_rho[country*3+mp],cor_theta1[country*3+mp],cor_theta2[country*3+mp],qstar[country*3+mp],θ[country*3+mp])=round.(Calibrate(mp,country,1),digits=3)
        end 
    end
    df.α=α
    df.s=s
